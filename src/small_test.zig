...
test "execute small test" {
    ...
    while (object.obj_type(&current) != .nil) {
        ...
        const expr_str = try format.write_to_slice(global_pool, object.get_car(&object.get_car(&current)), allocator);
        defer allocator.free(expr_str);
        const expected_str = try format.write_to_slice(global_pool, object.get_car(&object.get_cdr(&object.get_car(&current))), allocator);
        ...
        if (! object.equal(&expected, &result)) {
            const formatted = try format.write_to_slice(evaluator.pool, expr, allocator);
            defer allocator.free(formatted);
            const result_formatted = try format.write_to_slice(evaluator.pool, result, allocator);
            defer allocator.free(result_formatted);
            const expected_formatted = try format.write_to_slice(evaluator.pool, expected, allocator);
