...
pub fn parse_number(p: *Parser, pool: *object.ObjPool) !*object.ObjHeader {
    var val: i32 = 0;
...
pub fn parse_list(p: *Parser, pool: *object.ObjPool) anyerror!*object.ObjHeader {
    if (p.s[p.p] == ')') {
        p.p+=1;
...
pub fn parse_simple_symbol(p: *Parser, pool: *object.ObjPool, start_pos: usize) anyerror!*object.ObjHeader {
...
pub fn parse_datum(p:*Parser, pool: *object.ObjPool) anyerror!*object.ObjHeader {
    if (p.s[p.p] == '#') {
        if (p.s[p.p + 1] == 't') {
            p.p+=2;
